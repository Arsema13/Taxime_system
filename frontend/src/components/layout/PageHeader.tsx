  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div className="min-w-0">
          ...
          <nav className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-500 mb-1.5 font-medium">
            {breadcrumbs.map((b, i) => (
              <React.Fragment key={i}>
                ...
                {b.to ? (
                  <Link to={b.to} className="hover:text-[#FF4D67] transition-colors truncate">{b.label}</Link>
                ) : (
        ...
        {title && <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-slate-100 tracking-tight truncate">{title}</h1>}
