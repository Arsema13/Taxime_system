...
import { Menu, Bell, Search, X, ChevronRight, Share2, Mail } from 'lucide-react';
...
export function Header({ onMenuToggle, title }: HeaderProps) {
  ...
  return (
    <header className="flex items-center justify-between px-6 shrink-0 z-20">
      <div className="flex items-center space-x-2">
        <Link to="/dashboard" className="w-10 h-10 bg-black rounded-xl flex items-center justify-center text-white font-bold text-xl hover:bg-slate-800 transition-colors">
          T
        </Link>
        <span className="font-bold text-xl tracking-tight text-slate-800">Taxime</span>
      </div>
      ...
      <nav className="hidden md:flex space-x-6 text-sm font-medium text-slate-600">
        ...
        <Link to="/tasks" className="hover:text-black transition-colors">Tasks</Link>
        <Link to="/teams" className="hover:text-black transition-colors">Teams</Link>
        <Link to="/reports" className="hover:text-black transition-colors">Reports</Link>
        <Link to="/calendar" className="hover:text-black transition-colors">Calendar</Link>
        <Link to="/notifications" className="hover:text-black transition-colors">Notifications</Link>
      </nav>

      <div className="flex items-center space-x-3">
        <button
          onClick={onMenuToggle}
          className="lg:hidden p-2 bg-white/60 backdrop-blur-md rounded-full shadow-sm hover:bg-white transition-colors"
        >
          <Menu size={18} />
        </button>
        ...
        <div className="relative" ref={searchRef}>
          {searchOpen ? (
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-64 pl-9 pr-4 py-2 text-sm rounded-full border border-slate-200 bg-white/60 backdrop-blur-md focus:outline-none focus:ring-2 focus:ring-teal-500"
                />
              </div>
            ...
            <button
              ...
              className="p-2 bg-white/60 backdrop-blur-md rounded-full shadow-sm hover:bg-white transition-colors"
            >
              <Search size={18} />
            </button>
          )}
          ...
          {searchOpen && (searchQuery.trim()) && (
            <div className="absolute right-0 top-full mt-2 w-96 bg-white/90 backdrop-blur-md rounded-2xl border border-white/40 shadow-lg py-1 z-50">
              {searching ? (
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className="relative p-2 bg-white/60 backdrop-blur-md rounded-full shadow-sm hover:bg-white transition-colors"
          >
            <Bell size={18} />
            {unreadCount > 0 && (
          ...
          {notifOpen && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-white/90 backdrop-blur-md rounded-2xl border border-white/40 shadow-lg z-50 overflow-hidden">
