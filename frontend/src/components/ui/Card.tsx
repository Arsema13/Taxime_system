...
export function ErrorState({ message = 'Something went wrong.', onRetry }: ErrorStateProps) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
      <div className="w-16 h-16 bg-[#e89b1a]/10 backdrop-blur-md rounded-2xl flex items-center justify-center border border-[#e89b1a]/20">
        <svg className="w-8 h-8 text-[#e89b1a]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {onRetry && (
        <button
          onClick={onRetry}
          className="mt-1 px-4 py-2 rounded-xl bg-[#e89b1a] text-white text-sm font-medium hover:bg-[#d48a15] transition-colors"
        >
          Try again
        </button>
      )}
